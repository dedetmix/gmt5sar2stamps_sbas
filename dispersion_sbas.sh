...
do
  ...
  if [ -e $prm ]; then 
    if [ ! -e $name".grd" ]; then
      ...
      conv 1 1 $filter $prm $name".grd"  #2-D image convolution
...
gmt grdimage $outgrd -Cscatter.cpt -JX6i -P -X1i -Y3i -V -Bf500a1000WSen -K > scatter.ps
gmt psscale -D3/-0.8/5/0.5h -Cscatter.cpt -Ba0.2f0.1:"amplitude difference dispersion": -O >> scatter.ps
